<?php
	...
	$sql2 = "SELECT * FROM provincia LIMIT " . (($paginacion->get_page() - 1) * $resultados) . ', ' . $resultados;
			...
			<div class="table-responsive">
				<table class="table table-striped">
					...
					<tr style="text-align:center;">
						...
						<td>
							<button class="btn btn-primary" type="button" onclick="editar('.$row2['pv_id'].')">Editar</button>
						</td>
					</tr>
					';
					}
...
function nuevo(){
	window.location = '?modulo=nuevaprovincia';
}

function editar(data){
	window.location = '?modulo=editarprovincia&data='+data;
}
</script>