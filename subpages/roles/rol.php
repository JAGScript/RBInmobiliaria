<?php
	...
	$sql2 = "SELECT * FROM rol LIMIT " . (($paginacion->get_page() - 1) * $resultados) . ', ' . $resultados;
			...
			<div class="table-responsive">
				<table class="table table-striped">
					...
					<tr style="text-align:center;">
						...
						<td>
							<button class="btn btn-primary" type="button" onclick="editar('.$row2['ro_id'].')">Editar</button>
						</td>
					</tr>
					';
					}
...
$(document).ready(function(){
	$('#titulopag').html('<div class="col-md-11">\
							<h3>\
								...
								Lista de Roles\
								<button class="btn btn-success pull-right" type="button" onclick="nuevo()">Nuevo Rol</button>\
							</h3>\
						</div>');
});

function nuevo(){
	window.location = '?modulo=nuevorol';
}

function editar(data){
	window.location = '?modulo=editarrol&data='+data;
}
</script>