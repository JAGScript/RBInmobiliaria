<?php 
	...
	$select = "SELECT * FROM tipo_propiedad WHERE tp_id = ?";
...
<div class="row" style="margin:25px 0px 75px 0px;">
	<div class="col-md-8 col-md-push-2" style="border:2px solid #ccc; border-radius:10px; padding:20px;">
		<div class="row">
			...
			<div class="col-md-9">
				<input type="text" id="nombre" class="form-control" placeholder="Nombre Tipo Propiedad" value="<?php echo $row['tp_nombre'];?>" />
			</div>
		</div>
		<div class="row">
			<div class="col-md-3">
			...
			<div class="col-md-9">
				<?php if($row['tp_estado'] == 1){?>
				<button type="button" class="btn btn-default" onclick="activacion('<?php echo $row['tp_estado'];?>')">
...
function volver(){
	window.location = '?modulo=listatipopropiedad';
}
...
function guardar(data){
	...
	if(nombre == ''){
		...
		$.post('subpages/tipopropiedad/ajax/guardareditar.php',{
