<?php
	...
	$sql1 = "SELECT * FROM canton JOIN provincia ON ca_provincia_id = pv_id";
	...
	$sql2 = "SELECT * FROM canton JOIN provincia ON ca_provincia_id = pv_id LIMIT " . (($paginacion->get_page() - 1) * $resultados) . ', ' . $resultados;
			...
			<div class="table-responsive">
				<table class="table table-striped">
					...
					<tr style="text-align:center;">
						...
						<td>
							<button class="btn btn-primary" type="button" onclick="editar('.$row2['ca_id'].')">Editar</button>
						</td>
					</tr>
					';
					}
...
function nuevo(){
	window.location = '?modulo=nuevocanton';
}

function editar(data){
	window.location = '?modulo=editarcanton&data='+data;
}
</script>